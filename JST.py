...
class JST:
    ...
    # pendefinisian fungsi untuk melakukan proses denomarlisasi data
    ...
    def Output2Hidden(self,target_output,output,Z,alpha,W):
        try:
            ...
            for i in range(baris):
                for j in range(kolom):
                    tao[i,j]= (target_output[j]-output[i,j])*output[i,j]*(1-output[i,j])
            ...
            for i in range(kolom):
                for j in range (kolom1):
                    deltaW[j+1,i]=round(alpha*tao[0,i]*Z[0,j],3)
